...
class TikTokScraper extends events_1.EventEmitter {
    constructor({ download, filepath, filetype, proxy, asyncDownload, cli = false, event = false, progress = false, input, number, type, by_user_id = false, store_history = false, historyPath = '', userAgent, noWaterMark = false, fileName = '', timeout = 0, bulk = false, zip = false, test = false, hdVideo = false, signature = '', sign, webHookUrl = '', method = 'POST', }) {
        super();
        this.storeValue = '';
        this.sign = sign;
        ...
        for (let i = 0; i < posts.length; i += 1) {
            ...
            if (this.noDuplicates.indexOf(posts[i].id) === -1) {
                ...
                const item = Object.assign(Object.assign({ id: posts[i].id, text: posts[i].desc, createTime: posts[i].createTime, authorMeta: {
                    ...
                    }, webVideoUrl: `https://www.tiktok.com/@${posts[i].author.uniqueId}/video/${posts[i].id}`, videoUrl: posts[i].video.downloadAddr, videoUrlNoWaterMark: `https://api2.musical.ly/aweme/v1/playwm/?video_id=${posts[i].video.id}${this.hdVideo ? '&improve_bitrate=1&ratio=1080p' : ''}`, videoMeta: {
